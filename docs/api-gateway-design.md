...
graph TB
    ...
    Client -->|"GET /users/me\nAuthorization: Bearer ***" UserSvc["用户服务\nport: 3001"]
    Client -->|"POST /orders\nAuthorization: Bearer ***" OrderSvc["订单服务\nport: 3002"]
    Client -->|"GET /products\nAuthorization: Bearer ***" ProductSvc["商品服务\nport: 3003"]
...
graph TB
    ...
    Client -->|"所有请求\nAuthorization: Bearer ***" Gateway["🔵 API 网关\napi.example.com\nport: 3000"]
...
**Middleware（中间件）**：最先执行，可以访问 `req` 和 `res` 对象，执行完毕后必须调用 `next()` 传递控制权。适合做请求日志记录和 Trace ID 注入。
...
**Exception Filter（异常过滤器）**：捕获整个管道中抛出的任何异常，统一格式化错误响应。
...
sequenceDiagram
    ...
    Client->>MW: POST /api/orders Authorization: Bearer ***
...
- **水平扩展**：部署多个网关实例，在前面放负载均衡器
- **无状态设计**：网关本身不存储会话状态，任何实例都可以处理任何请求
- **限流计数存 Redis**：多个网关实例共享同一个 Redis，限流计数保持一致

### 网关延迟增加

网关主要做的是 JWT 签名验证（纯本地计算，约 0.5-2ms）、Redis 读取（约 1-3ms）、HTTP 转发（网络本身的延迟）。正常情况下，网关引入的额外延迟在 5ms 以内。
