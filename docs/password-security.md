...
现代 GPU（如 NVIDIA RTX 4090）使用 Hashcat 计算 SHA-256 的速度：约 **1500 亿～3000 亿次/秒（150～300 GH/s）**。

这意味着：

- 一张 RTX 4090 每秒可以尝试 1500 亿～3000 亿条密码
- 8 位数字密码（约 1 亿种组合）：不到 **0.0004 秒** 破解
- 8 位小写字母+数字密码（约 218 亿种）：约 **0.07～0.15 秒** 破解
...
彩虹表的核心思路就是这样：**把成千上万对"密码-哈希"压缩成一条条"链"，每条链只存首尾两端**。

具体来说，攻击者会从大量不同的随机起点出发，各自构造一条长链。整个彩虹表就是这些链的**起点-终点对的集合**。一条链可能走过几千步，覆盖几千对"密码-哈希"，但只需存储 2 个值（起点和终点）。
...
从一个起始密码出发，交替执行"哈希 → 归约 → 哈希 → 归约……"，就像一条流水线。**每一步使用不同的归约函数**（R1、R2、R3……），且在同一条链上每个归约函数只使用一次。链的长度是预先设定的固定值（比如 1000 步、10000 步），这里为了便于理解用 3 步举例：

```mermaid
flowchart LR
    P1["🟢 P1<br/>abc123"] --哈希--> H1["H1<br/>d93591..."]
    H1 --R1归约--> P2["P2<br/>djcfab"]
    P2 --哈希--> H2["H2<br/>a7f21c..."]
    H2 --R2归约--> P3["P3<br/>kqm019"]
    P3 -.->|"..."| H_final["🔴 H_final<br/>（预设步数后的<br/>最后一个哈希值）"]
```

链的终点不是归约结果，而是**预设步数走完后，最后一个哈希值**。具体构造过程如下：

1. 选起点：随机选一个起始密码 P1 = `abc123`
2. 第 1 步：P1 --哈希--> H1 = `d93591...`，H1 --R1归约--> P2 = `djcfab`
3. 第 2 步：P2 --哈希--> H2 = `a7f21c...`，H2 --R2归约--> P3 = `kqm019`
4. **预设步数走完后**：停止归约，最后的哈希值 H_final = `...` 就是终点

**彩虹表只存储每条链的起点（P1）和终点（H_final），中间全部丢弃。**

#### 如何用彩虹表破解一个哈希

假设数据库泄露，攻击者拿到了哈希值 `a7f21c...`，想还原出密码。彩虹表的工作分为**构造阶段**和**查询阶段**两个过程。

##### 阶段一：构造阶段（事先预计算）

攻击者预先构建彩虹表。假设链长预设 3 步（实际通常是几千步），构造过程如下。**"3 步"指的是 3 次哈希操作**，链的结构是 `P --H--> H --R--> P --H--> H --R--> P --H--> H`，最后停在哈希值上：
...
flowchart LR
    ...
    P2 --哈希--> H2["H2<br/>a7f21c..."]
    H2 --R2归约--> P3["P3<br/>kqm019"]
    P3 --哈希--> H3["🔴 H3（终点）<br/>6f9abc..."]
```

1. 选起点：随机选 P1 = `abc123`
2. 第 1 步：P1 --哈希--> H1 = `d93591...`，H1 --R1归约--> P2 = `djcfab`
3. 第 2 步：P2 --哈希--> H2 = `a7f21c...`，H2 --R2归约--> P3 = `kqm019`
4. 第 3 步（最后一步）：P3 --哈希--> H3 = `6f9abc...`，**停止归约**，H3 直接作为终点

**彩虹表只存储起点 `abc123` 和终点 `6f9abc...`**，中间所有值全部丢弃。

##### 阶段二：查询阶段（拿到目标哈希后）

假设目标哈希是 `a7f21c...`。查询过程分三步，每一步都有其明确的意义。

**第一步：在终点集合中查找落脚点**

:::details 理解什么是落脚点

在解释查询方法之前，先回到链的结构上。一条 3 步链长这样：

```
P1 --哈希--> H1 --R1归约--> P2 --哈希--> H2 --R2归约--> P3 --哈希--> H3（终点）
```

注意链中每一个**哈希值**出现的位置：

- H1 出现在第 1 步末尾，紧接着用 R1 归约
- H2 出现在第 2 步末尾，紧接着用 R2 归约
- H3 是第 3 步的最终结果，不做归约，直接作为终点存储
...
换句话说：**如果一个哈希值曾经出现在某条链的第 N 步末尾（即紧挨着 RN 归约之后），那么对这个哈希值做 RN 归约，得到的结果就是这个链上的下一个密码**。

这个"通过归约推导出来的密码"，就是我们要找的**落脚点**。

:::

现在回到目标哈希 `a7f21c...`（即 H2）。我们已经知道：

- 如果 H2 出现在某条链的第 2 步末尾，那么对 H2 做 R2 归约，应该能得到 P3
- P3 是这条链上第 3 步的起点

所以我们**先用 R2 归约目标哈希，得到候选落脚点 P3 = `kqm019`**，然后拿这个值去**终点集合中查找**——不是查 H3 本身，而是查 **P3 是否等于某条链的终点 H3**。

这个逻辑等价于在问："**有没有某条链，它的第 3 步终点正好是 P3？**"

终点集合是排序好的，用二分查找很快。如果 R2 归约结果没命中，说明目标哈希不是任何链的第 3 步哈希值。

**第二步：依次尝试所有可能的步数位置**

如果 R2 归约没命中，继续尝试 R1。这次我们问的是："**目标哈希是不是某条链的第 2 步末尾（即紧跟在 R1 归约之后）？**"

对 H2 做 R1 归约得到 P2 = `djcfab`，拿 P2 去终点集合中查找——**命中了！** 有一条约链的终点正好是 P2。

命中的意义是：这条链的起点是 `abc123`，且目标哈希 H2 确实出现在这条链的第 2 步。

**第三步：重走链验证并提取密码**

知道命中了哪条链后，从该链的起点 `abc123` 开始重新走一遍：

```
P1 --哈希--> H1 --R1归约--> P2 --哈希--> H2 ✅
```

走到第 2 步时，哈希值恰好等于目标哈希 `a7f21c...`，该位置的输入密码是 **`djcfab`**——这就是答案。

**总结查询的完整流程**

```
对目标哈希 H_target：
1. 尝试 R_n 归约 → 得到落脚点 P_candidate
2. 在终点集合中查找 P_candidate 是否等于某个终点 H_final
3. 如果命中：从该链的起点重走，定位到目标哈希，输入密码即为答案
4. 如果未命中：尝试 R_{n-1}，重复以上过程
```

这里的关键洞察是：**落脚点 P_candidate 和终点 H_final 是同一种东西（都是密码格式的字符串），所以可以用 P_candidate 直接去终点集合中做字符串比对。** 彩虹表存的是终点，但如果某个中间哈希值的"下一个密码"恰好等于某条链的终点，那这条链就包含了目标哈希。

#### 为什么重走链就能找到原始密码

关键在于哈希函数的**确定性**：对于同一个哈希算法和同一个输入，输出永远不变。所以从起点重走的每一步，产生的哈希值和构造时完全一致。如果目标哈希曾经出现在链的某个位置，重走时就一定会再次出现在同一个位置，该位置的输入密码就是答案。

#### 为什么需要知道哈希算法

彩虹表是**针对特定哈希算法**构建的。MD5 彩虹表只能破解 MD5 哈希，SHA-1 彩虹表只能破解 SHA-1 哈希。攻击者需要事先知道目标系统使用的是哪种哈希算法，然后使用对应的彩虹表。这也是为什么使用 Bcrypt、Argon2 等慢哈希算法非常有效——这些算法的彩虹表构建成本极高，在实际中几乎不可行。

::: details 为什么需要多组归约函数

如果只使用单一的归约函数 R，会有两个严重问题：

**问题一：碰撞合并**

假设链 A 和链 B 在某一步产生了相同的哈希值 H：

- 链 A：... → H --R--> P' → ...
- 链 B：... → H --R--> P' → ...

后续计算完全相同，两条链会**合并成一条**，导致覆盖的密码范围大幅缩小。

**问题二：中间哈希无法查询**

即使目标哈希恰好等于某个中间哈希值 H*i，直接用 R(H_i) 去查表几乎不可能命中——R(H_i) 产生的是密码值 P*{i+1}，而表中存的是哈希值。

使用多组归约函数解决了这两个问题：不同位置用不同的归约函数，即使哈希值相同，归约结果也不同，不会合并；查询时依次尝试不同位置的归约函数，总能找到匹配的链。
...
OWASP 给出了 Argon2id 的最低安全阈值（19 MiB 内存、迭代 2、并行度 1），以下是可以等效交换 CPU/内存消耗的几组配置示例：

| 内存（m） | 迭代（t） | 并行（p） | 说明           |
| --------- | --------- | --------- | -------------- |
| 19 MB     | 2         | 1         | OWASP 最低要求 |
| 46 MB     | 1         | 1         | 等效配置       |
| 64 MB     | 3         | 1         | 适中配置       |
| 256 MB    | 4+        | 1         | 高安全场景     |
...
async function hashPassword(password: string): Promise<string> {
  return argon2.hash(password, {
    ...
    timeCost: 3, // 迭代 3 次
    parallelism: 1, // 1 个线程（OWASP 最低要求）
  });
}
...
| 适用场景    | 遗留系统（无法使用 Argon2 时） | 新项目首选，特别是高安全要求场景 |

**建议**：

- **新项目**：优先使用 Argon2id
- **已有 Bcrypt 的项目**：不必立即迁移，Bcrypt 仍然安全；下节介绍如何优雅迁移
- **遗留系统**：如果当前使用 Bcrypt 且运行正常，可以继续使用；但新系统应将 Argon2id 作为首选
...
美国国家标准与技术研究所（NIST）在 SP 800-63B 中提供了密码策略的权威指南，该版本（2017 年发布，2020 年更新 errata）推翻了很多"传统认知"：
