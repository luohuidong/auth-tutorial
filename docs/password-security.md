...
**彩虹表只存储起点 `abc123` 和终点 `6f9abc...`**，中间所有值（P2、H1、P3、H2、R1、R2）全部丢弃。

**阶段二：查询阶段（拿到目标哈希后）**

假设目标哈希是 `a7f21c...`（即 H2）。查询过程是：**先检查是否是终点，再从起点重走比对**。

**第 1 步：检查是否是终点**

对 H2 = `a7f21c...` 做 R2 归约，得到 P3 = `kqm019`。彩虹表中存的是哈希值终点，不是密码值，搜不到。

**第 2 步：从起点重走，边走边比对**

上一步没命中，说明目标哈希不是链的终点。要检查它是否在链的中间位置，方法是：**从起点重走这条链，每走一步就检查当前哈希是否等于目标哈希 H2**：

- 第 1 步重走：P1 --哈希--> H1 = `d93591...`，H1 ≠ H2
- 第 2 步重走：H1 --R1归约--> P2 --哈希--> H2 = `a7f21c...`，**H2 = H2，找到！**

找到了目标哈希 H2，它的输入密码就是 P2 = **`djcfab`** ✅

**另一种情况：目标哈希恰好是某条链的终点**

如果目标哈希 `a7f21c...` 正好是另一条链的**终点**，那就不需要重走，直接命中：

- 在彩虹表中搜索 `a7f21c...` → **命中！** 查到这条链的起点是 `abc123`
- 从 `abc123` 出发重走，直到哈希值等于 `a7f21c...`
- `abc123` --哈希--> `d93591...`（H1）
- H1 --R1归约--> `djcfab`（P2）
- P2 --哈希--> `a7f21c...`（H2）= 目标哈希 ✅

目标哈希 `a7f21c...` 对应的输入密码是 **`djcfab`** ✅
