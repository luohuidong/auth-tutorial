...
想象你是一家银行的设计师，需要存放客户的物品（密码）。你有几种方案：

**方案一：把原物放进抽屉**

这就是明文存储密码。数据库被盗，所有密码立刻暴露。就像把客户的现金直接放在贴着名字标签的抽屉里——抽屉被撬，现金全没。

**方案二：把物品装进加密保险柜**

这是"加密"密码。保险柜被撬走后，如果攻击者找到了主密钥（密钥存在服务器上！），照样能打开。更糟的是，主密钥泄露后，所有保险柜都可以被打开。

**方案三：把物品粉碎后存入**

这才是"哈希"密码。物品经过不可逆的粉碎机处理，你存的是碎片而非原物。银行自己也不知道原物是什么，只能把客户下次带来的东西再过一遍粉碎机，然后比对碎片形状是否一致。

**方案四：给每人用不同模式的粉碎机**

这是"加盐哈希"。即使两个客户带来了一模一样的东西，因为用了不同的粉碎机（不同的盐），碎片形状完全不同。攻击者即使知道某种碎片对应什么原物，也没法批量还原。

**方案五：把物品先过粉碎机，然后一次次重新粉碎一万遍**

这是"慢哈希算法"如 Bcrypt 和 Argon2 的核心思想。正常用户每次登录多等几百毫秒，不影响体验；但攻击者想批量暴力破解时，每次尝试都要等那么久，计算成本会让攻击在能量消耗上变得不可行。
...
2019 年，Facebook 被曝出以明文形式将数亿账号密码存储在内部日志文件中，影响超过 6 亿用户。2016 年，LinkedIn 数据库泄露了 1.17 亿个 MD5 哈希密码，因为 MD5 太弱，这些密码在数小时内被大量破解。2013 年，Adobe 因错误地使用 ECB 模式加密（而不是哈希）存储 1.5 亿个密码，导致相同密码在数据库中呈现相同的密文，攻击者借助模式分析轻松还原了大量密码。
...
暴力破解虽然快，但在线实时破解仍然受网络速度和账号锁定限制。聪明的攻击者发现了更高效的方式：**预计算**。

**朴素预计算（查找表）**

攻击者事先将常见密码（字典中的词、所有 8 位数字字母组合等）全部哈希好，存成"密码 → 哈希值"的映射表。当拿到一批泄露的哈希值时，只需查表，瞬间得到对应的明文密码。

问题是：8 位所有字符组合的查找表可能有几 TB，存储成本极高。

**彩虹表（Rainbow Table）：时间-空间的权衡**

彩虹表是一种改进的预计算方案，通过"归约函数"将哈希和密码交替链接，在存储空间和查找时间之间取得平衡，可以用几 GB 的空间覆盖本来需要 TB 级存储的查找表。
...
- 网站 [hashes.com](https://hashes.com) 提供免费的 MD5 和 SHA-1 彩虹表查询
- 对于无盐 MD5 哈希，90% 以上的 8 位以下密码可以在几秒内被逆向还原
- "password" 的 MD5 哈希（5f4dcc3b5aa765d61d8327deb882cf99）在全球任何彩虹表里都有记录
