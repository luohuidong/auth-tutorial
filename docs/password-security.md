...
从一个起始密码出发，交替执行"哈希 → 归约 → 哈希 → 归约……"，就像一条流水线。链的长度是预先设定的固定值（比如 1000 步、10000 步），这里为了便于理解用 3 步举例：

```mermaid
flowchart LR
    P1["🟢 P1<br/>abc123"] --哈希--> H1["H1<br/>d93591..."]
    H1 --归约--> P2["P2<br/>djcfab"]
    P2 --哈希--> H2["H2<br/>a7f21c..."]
    H2 --归约--> P3["P3<br/>kqm019"]
...
2. **第 1 步**：P1 --哈希--> H1 = `d93591...`，H1 --归约--> P2 = `djcfab`
3. **第 2 步**：P2 --哈希--> H2 = `a7f21c...`，H2 --归约--> P3 = `kqm019`
...
攻击者预先构建彩虹表。假设链长预设 2 步（实际通常是几千步），构造过程如下：

```mermaid
flowchart LR
    P1["🟢 P1<br/>abc123"] --哈希--> H1["H1<br/>d93591..."]
    H1 --归约(R1)--> P2["P2<br/>djcfab"]
    P2 --哈希--> H2["H2<br/>a7f21c..."]
    H2 --归约(R2)--> P3["P3<br/>kqm019"]
    P3 --哈希--> H3["H3<br/>6f9abc..."]
    H3 -.->|"终点"| H_final["🔴 H_final<br/>6f9abc..."]
```

1. 选起点：随机选 P1 = `abc123`
2. 第 1 步：P1 --哈希--> H1 = `d93591...`，R1(H1) = `djcfab` = P2
3. 第 2 步：P2 --哈希--> H2 = `a7f21c...`，R2(H2) = `kqm019` = P3
4. 预设步数走完，H3 = `6f9abc...` 作为终点

**彩虹表只存储起点 `abc123` 和终点 `6f9abc...`**，中间所有值都丢弃。

**阶段二：查询阶段（拿到目标哈希后）**

假设目标哈希是 `a7f21c...`（即 H2）。查询过程如下：

**第 1 步：尝试用 R2 归约目标哈希**

对 `a7f21c...` 做 R2 归约，得到 `kqm019`。在彩虹表中搜索——但 `kqm019` 是密码值 P3，而彩虹表只存了哈希值作为终点，所以搜不到。

**第 2 步：换个归约函数再试**

换用 R1 对目标哈希做归约：R1(`a7f21c...`) = `6f9abc...`。在彩虹表中搜索 `6f9abc...`——找到了！这是某条链的**终点**。查到这条链的起点是 `abc123`。
