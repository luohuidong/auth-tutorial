...
注意：归约函数 **不是** 哈希的逆运算——它不能还原出原始密码。它只是一种人为定义的规则，从哈希值中提取一些数字和字母，拼成一个"看起来像密码"的字符串。

举个例子，一种简单的归约函数可以这样工作：取哈希值的前 6 个字符，按某种规则映射成字母和数字：

```mermaid
flowchart LR
    H["哈希值<br/>d93591bce8a7f21c..."]
    R["归约函数"]
    P["新字符串<br/>djcfab"]
    H --> R --> P
```

归约函数做的事很简单：把一串哈希值通过某种规则"翻译"成一个新的字符串。这个字符串**不是原始密码**，只是碰巧看起来像密码而已。
