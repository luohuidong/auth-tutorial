...
网络抖动和多标签页并发刷新的根源在前端没有协调好，解决方案是在前端加锁——多个并发请求只允许一个真正发出刷新请求，其他的等待结果：
...
**后端的正确处理：如果检测到令牌重用，说明发生了令牌泄露，必须吊销所有会话**：

```typescript
async function refresh(oldRefreshToken: string): Promise<TokenPair> {
  const tokenRecord = await db.findRefreshToken(oldRefreshToken);

  // 检测到令牌重用——可能是攻击者在用偷来的 Token
  if (tokenRecord.replacedAt) {
    // 吊销该用户所有会话，要求重新登录
    await db.revokeAllUserSessions(tokenRecord.userId);
    throw new Error('令牌泄露，已吊销所有会话');
  }

  // 正常情况：删除旧 Token，颁发新 Token
  await db.deleteRefreshToken(oldRefreshToken);
  const newToken = await issueNewTokenPair(tokenRecord.userId);
  return newToken;
}
```

关键点：**不要试图"宽容"处理令牌重用**。如果收到已被替换的旧 Token，说明要么是正常并发刷新（前端刷新锁可以消除），要么是令牌泄露（必须立即吊销）。试图返回已颁发的新 Token 给攻击者和受害者，会让攻击者同样获得有效访问。
